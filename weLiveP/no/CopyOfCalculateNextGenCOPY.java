...
public class CopyOfCalculateNextGenCOPY {

	public CopyOfCalculateNextGenCOPY(){
	...
	public ArrayList<UsersPoints> nextGeneration() {
		...
		for(UsersPoints p : OldUsersPointsArray){
			...
			//Count neighbors
//			neighbor = countNeighbors(OldUsersPointsArray, x, y);
...
//					neighbor++;
//				}
				if(hasNeighbor( x, y, t.getX(), t.getY() )){
					neighbor++;
				}
			}
...
//					if(x == d.getX() && y == d.getY()){
	...
	private int countNeighbors(ArrayList<UsersPoints> OldUsersPointsArray, int x, int y){
		int neighbor = 0;
		
		for(UsersPoints t : OldUsersPointsArray){
			if(
					(x == t.getX()-1 && (y == t.getY()-1 || y == t.getY() || y == t.getY()+1)) ||
					(x == t.getX()+1 && (y == t.getY()-1 || y == t.getY() || y == t.getY()+1)) ||
					(x == t.getX()) && (y == t.getY()-1|| y == t.getY()+1)||
					(y == t.getY()-1 && (x == t.getX()-1 || x == t.getX() || x == t.getX()+1)) ||
					(y == t.getY()+1 && (x == t.getX()-1 || x == t.getX() || x == t.getX()+1)) ||
					(y == t.getY() && (x == t.getX()-1 || x == t.getX()+1))
					){
				neighbor++;
			}
		}
